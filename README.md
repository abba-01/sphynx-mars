...
- `analysis/FIXING_THE_ZERO_POINT.md` + `analysis/corrected_range_chart.py` — **why the range origin cannot be repaired by moving it**: the true reference is the horizon (a singularity, 35.7 in from where zero was placed), the mapping is hyperbolic at every origin, and — read correctly against that reference — the annotation's own anchor marks measure the peaks at 29.6 m / 29.2 m (agreeing to 1%, within 1 product px of the machine) — plus the corrected near-field range chart that replaces the linear model
...
- `analysis/OBJECT_X_VS_SPHINX.md` + `analysis/objectx_vs_sphinx_scale.png` — **Object X sized against the Great Sphinx with a range-free upper bound**: even at NASA's own stated maximum range for the hummock field (1006 m), Object X tops out at 46% of the Sphinx's length / 71% of its height, and matching either dimension exactly requires a range 1.4–2.2× beyond that stated maximum
